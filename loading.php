...
$query_2 = "SELECT L.ID, L.CoilID, format(L.ScanDate, 'dd/MM/yyyy') as OnlyDate, format(L.ScanDate, 'hh:mm:ss tt') as OnlyTime,
L.FormerNo, L.NominalSize, L.GradeName, L.FormerWeight, L.Netweight, L.Grossweight, L.Action, L.GI_No, R.ActionName
FROM Tbl_Data_Loading L
LEFT JOIN Tbl_Info_Action R
...
<div class="wrapper">
	...
	<p id="success"></p>
        <div class="table-wrapper">
            ...
            <div class="tbl-top">
                    <a href="/index.php"> UNASSIGNED </a>
                    <form name="theform_1" method="POST" action="/loading.php">
                        <input type="hidden" name="action" value="1">
                        <a href="javascript:document.theform_1.submit()" class="<?php if($action==1){echo 'open';}?>">CUTTING STATION 1</a>
                    </form>
                    <form name="theform_2" method="POST" action="/loading.php">
                        <input type="hidden" name="action" value="2">
                        <a href="javascript:document.theform_2.submit()" class="<?php if($action==2){echo 'open';}?>">CUTTING STATION 2</a>
                    </form>
                    <form name="theform_3" method="POST" action="/loading.php">
                        <input type="hidden" name="action" value="3">
                        <a href="javascript:document.theform_3.submit()" class="<?php if($action==3){echo 'open';}?>">CUTTING STATION 3</a>
                    </form>
                    <form name="theform_4" method="POST" action="/loading.php">
                        <input type="hidden" name="action" value="4">
                        <a href="javascript:document.theform_4.submit()" class="<?php if($action==4){echo 'open';}?>">CUTTING STATION 4</a>
                    </form>
                    <form name="theform_5" method="POST" action="/loading.php">
                        <input type="hidden" name="action" value="5">
                        <a href="javascript:document.theform_5.submit()" class="<?php if($action==5){echo 'open';}?>">SPOOLING</a>
                    </form>
                    <form name="theform_6" method="POST" action="/loading.php">
                        <input type="hidden" name="action" value="9">
                        <a href="javascript:document.theform_6.submit()" class="<?php if($action==9){echo 'open';}?>">HOLD</a>
                    </form>
                    <form name="theform_7" method="POST" action="/loading.php">
                        <input type="hidden" name="action" value="7">
                        <a href="javascript:document.theform_7.submit()" class="<?php if($action==7){echo 'open';}?>">REJECT</a>
                    </form>
                </div>
                <div class="drop_down">
            ...
            <table class="table table-striped table-hover">
				...
				?>
                <?php
					while($row_2 = sqlsrv_fetch_array($result_2)) {
				?>
                    <tr id="<?php echo $row_2["ID"]; ?>">
                        <td><?php echo $i; ?></td>
                        <td><?php echo $row_2["OnlyDate"]; ?></td>
                        <td><?php echo $row_2["OnlyTime"]; ?></td>
                        <td><?php echo $row_2["CoilID"]; ?></td>
                        <td><?php echo $row_2["FormerNo"]; ?></td>
                        <td><?php echo $row_2["Grossweight"]; ?></td>
                        <td><?php echo $row_2["FormerWeight"]; ?></td>
                        <td><?php echo $row_2["Netweight"]; ?></td>
                        <td><?php echo $row_2["GI_No"]; ?></td>
				    	<td><?php echo $row_2["NominalSize"]; ?></td>
				    	<td><?php echo $row_2["GradeName"]; ?></td>
                        <td>
				    		<span class="custom-checkbox">
				    			<input type="checkbox" class="user_checkbox" data-user-id="<?php echo $row_2["ID"]; ?>">
				    			<label for="checkbox2"></label>
				    		</span>
				        </td>
			        </tr>
